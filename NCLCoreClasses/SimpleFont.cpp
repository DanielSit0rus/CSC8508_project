...
void SimpleFont::BuildVerticesForString(const std::string& text, const Vector2& startPos, const Vector4& colour, float size, std::vector<Vector3>& positions, std::vector<Vector2>& texCoords, std::vector<Vector4>& colours) {
	...
	for (size_t i = 0; i < text.length(); ++i) {
		...
		float scale = size;
		float charWidth = static_cast<float>(charData.x1 - charData.x0) / texWidth * scale;
		...
		positions.emplace_back(Vector3(startPos.x + xStart, yStart + yOff, 0.0f));
		positions.emplace_back(Vector3(startPos.x + xStart, yStart + yOff - yHeight, 0.0f));
		positions.emplace_back(Vector3(startPos.x + xStart + charWidth, yStart + yOff - yHeight, 0.0f));

		positions.emplace_back(Vector3(startPos.x + xStart + charWidth, yStart + yOff - yHeight, 0.0f));
		positions.emplace_back(Vector3(startPos.x + xStart + charWidth, yStart + yOff, 0.0f));
		positions.emplace_back(Vector3(startPos.x + xStart, yStart + yOff, 0.0f));

		colours.insert(colours.end(), 6, colour);
...
void SimpleFont::BuildInterleavedVerticesForString(const std::string& text, const Vector2& startPos, const Vector4& colour, float size, std::vector<InterleavedTextVertex>& vertices) {
	int endChar = startChar + numChars;
	float currentX = 0.0f;
	InterleavedTextVertex verts[6];
	...
	for (size_t i = 0; i < text.length(); ++i) {
		...
		verts[0].pos = Vector3(startPos.x + xStart, yStart + yOff, 0.0f);
		verts[1].pos = Vector3(startPos.x + xStart, yStart + yOff - yHeight, 0.0f);
		verts[2].pos = Vector3(startPos.x + xStart + charWidth, yStart + yOff - yHeight, 0.0f);

		verts[3].pos = Vector3(startPos.x + xStart + charWidth, yStart + yOff - yHeight, 0.0f);
		verts[4].pos = Vector3(startPos.x + xStart + charWidth, yStart + yOff, 0.0f);
		verts[5].pos = Vector3(startPos.x + xStart, yStart + yOff, 0.0f);

		for (int j = 0; j < 6; ++j) {
			verts[j].colour = colour;
			verts[j].texCoord = Vector2(charData.x0 * texWidthRecip + (j % 3 == 2 ? charWidth : 0), charData.y0 * texHeightRecip + ((j == 1 || j == 2 || j == 5) ? yHeight : 0));
		}
