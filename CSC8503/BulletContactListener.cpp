...
void BulletContactListener::onTrigger(const rp3d::OverlapCallback::CallbackData& callbackData)
{
	for (uint32_t i = 0; i < callbackData.getNbOverlappingPairs(); i++) {
		...
		if (object1->GetType() == GameObjectType::bullet && object2->GetType() == GameObjectType::player) {
			...
			// Destroy the bullet after impact
			((PaintballBullet*)object1)->Destroy();
		}else if (object1->GetType() == GameObjectType::bullet && object2->GetType() == GameObjectType::coward) {
			PaintballCoward* enemy = static_cast<PaintballCoward*>(object2);
		...
		else if (object2->GetType() == GameObjectType::bullet && object1->GetType() == GameObjectType::coward) {
			PaintballCoward* enemy = static_cast<PaintballCoward*>(object1);
