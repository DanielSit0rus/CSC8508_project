	...
	for (auto i = first; i != last; ++i) {
		...
		for (auto j = i + 1; j != last; ++j) {
			...
			if(CollisionDetection::ObjectIntersection(*i, *j, info)){
				if (((*i)->GetName() == "MAMA_CAT" && (*j)->GetName() == "Kitten") && (*j)->isFollowing != true)
				{
					std::cout << "\n setting true\n";
					(*j)->isFollowing = true;
				}
				
				if (((*i)->GetName() == "Kitten" && (*j)->GetName() == "home") && (*i)->isFollowing == true && (*i)->athome == false )
				{
					score++;
					std::cout << "\n kitten got home\n";
					(*i)->isFollowing = false;
				...
				if (((*i)->GetName() == "Goose" && (*j)->GetName() == "MAMA_CAT") || ((*i)->GetName() == "MAMA_CAT" && (*j)->GetName() == "Goose"))
				{
					//score++;
					std::cout << "\n OUCH!!!!\n";
					Debug::Print("OOUUCCHHH!!!", Vector2(50, 50));
					catded = true;
					//(*i)->isFollowing = false;
				}
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	// Calculate tangential velocity (friction)
	Vector3 tangent = contactVelocity - (p.normal * Vector::Dot(contactVelocity, p.normal));
	float tangentLength = Vector::Length(tangent);
	if (tangentLength > 0.0f) {
		 tangent = Vector::Normalise(tangent);
	}

	// Friction coefficients
	float frictionStatic = 0.5f;  // Static friction coefficient
	float frictionDynamic = 0.3f; // Dynamic friction coefficient

	// Tangential impulse calculation
	float jt = -Vector::Dot(contactVelocity, tangent);
	...
	float frictionFactor = static_cast<float>(frictionStatic * fabs(j));
	Vector3 maxFrictionImpulse = tangent * frictionFactor;
